(() => {
  ...
  // src/constants.js
  var SERVICE_UUID_BASE = (x) => `0000${x}-0000-1000-8000-00805f9b34fb`.toLowerCase();
  var SERVICE_UUIDS = {
  ...
  var CHUNK_COMMANDS = {
    READ: 1,
    BATTERY_GET_STATUS: 3
  };
  var CHUNK_RESPONSES2 = {
    RESULT_OK: 4
  };
  var FETCH_COMMANDS = {
    FROM_DATE: 1,
    BEGIN_TRANSFER: 2,
    ACKNOWLEDGE_AND_DROP: 769,
    ACKNOWLEDGE_NO_DROP: 777
  };
  var FETCH_DATA_TYPES = {
    ACTIVITY_DATA: 1,
    SP02_DATA: 37
  };
  ...
  function convertToInt32Array(bytes) {
    var result = [];
  ...
  function convertToInt16Array(bytes) {
    var result = [];
    for (var i = 0; i < bytes.length; i += 2) {
      result.push(bytes[i] << 8 | bytes[i + 1]);
    }
    return result;
  }
  ...
  function dateToUTCWatchDateArray(date) {
  ...
  var invertDictionary = (data) => Object.fromEntries(Object.entries(data).map(([key, value]) => [value, key]));
  ...
  var Authenticator = class extends EventTarget {
    constructor(band) {
      super();
      var self = this;
      this.Band = band;
    ...
    async authenticate() {
      ...
      this.pub = Module.HEAPU8.subarray(this.pub_buf, this.pub_buf + this.ECC_PUB_KEY_SIZE);
      this.prv = Module.HEAPU8.subarray(this.prv_buf, this.prv_buf + this.ECC_PRV_KEY_SIZE);
      ...
      await this.Band.writeChunkedValue(this.Band.Chars.CHUNKED_WRITE, CHUNK_ENDPOINTS2.AUTH, this.Band.getNextHandle(), Uint8Array.from(auth));
    ...
    async onChunkedRead(e) {
      ...
      if (value.length > 1 && value[0] == 3) {
        ...
        let headerSize;
        if (sequenceNumber == 0 && value[9] == CHUNK_ENDPOINTS2.AUTH && value[10] == 0 && value[11] == 16 && value[12] == 4 && value[13] == 1) {
          this.reassembleBuffer_pointer = 0;
          headerSize = 14;
          ...
          headerSize = 5;
        } else if (value[9] == CHUNK_ENDPOINTS2.AUTH && value[10] == 0 && value[11] == 16 && value[12] == 5 && value[13] == 1) {
        ...
        if (this.reassembleBuffer_pointer == this.reassembleBuffer_expectedBytes) {
          ...
          const rpub = Module.HEAPU8.subarray(rpub_buf, rpub_buf + this.ECC_PUB_KEY_SIZE);
          rpub.set(remotePublicEC);
          const sec_buf = Module._malloc(this.ECC_PUB_KEY_SIZE);
          const sec = Module.HEAPU8.subarray(sec_buf, sec_buf + this.ECC_PUB_KEY_SIZE);
          ...
          if (out1.length == 16 && out2.length == 16) {
            ...
            await this.Band.writeChunkedValue(this.Band.Chars.CHUNKED_WRITE, CHUNK_ENDPOINTS2.AUTH, this.Band.getNextHandle(), command);
          }
        }
        return true;
      }
  ...
  var Sp02Data = class {
    ...
    parseData(rawData) {
      ...
      var newData = [];
      for (let i = 1; i < rawData.length - 1; i += stride) {
        ...
        newData = newData.concat([
          {
            date,
            samples,
            average: nonzero_samples.reduce((a, b) => a + b) / nonzero_samples.length
          }
        ]);
      }
      if (newData.length == 0) {
      ...
      if (this.History.length > 0)
        this.NewestDate = this.History[this.History.length - 1].date;
    }
  ...
  // src/components/sp02.js
  var sp02Reader = class extends EventTarget {
    constructor(band) {
      super();
      this.Band = band;
      this.status = {
    ...
    async readSince(datetime) {
      ...
      await this.Band.Chars.FETCH.writeValueWithoutResponse(new Uint8Array([FETCH_COMMANDS.FROM_DATE, FETCH_DATA_TYPES.SP02_DATA, ...datetime, 0]));
      await new Promise((resolve, reject) => {
    ...
    async onFetchRead(e) {
      ...
      var raw2 = bufferToUint8Array(e.target.value);
      if (arraysEqual(raw2.slice(0, 3), new Uint8Array([16, 1, 1]))) {
        this.rawSp02Data = new Uint8Array();
        console.warn("Fetch Ready");
        this.dispatchEvent(new CustomEvent("fetch_start", {detail: true}));
        return true;
      } else if (arraysEqual(raw2.slice(0, 3), new Uint8Array([16, 2, 1]))) {
        await this.Band.Chars.FETCH.writeValueWithoutResponse(Uint8Array.from([3, 9]));
    ...
    async onActivityRead(e) {
      ...
      var raw2 = bufferToUint8Array(e.target.value);
      this.status.currentBatch = raw2[0];
      this.status.version = raw2[1] > 0 ? raw2[1] : this.status.version;
      var data = raw2.slice(1);
      var stride = 65;
  ...
  // src/models/battery.js
  var BatteryData = class {
    constructor(rawData) {
    ...
    parseData() {
      ...
      } else if (this.rawData.length == 32) {
  ...
  var batteryReader = class extends EventTarget {
    constructor(band) {
      super();
      this.Band = band;
    ...
    async Read() {
      ...
      await this.Band.writeChunk(CHUNK_ENDPOINTS2.BATTERY, [CHUNK_COMMANDS.BATTERY_GET_STATUS]);
      await new Promise((resolve, reject) => {
    ...
    parseRaw(raw2) {
      ...
      if (raw2.length == 32) {
        var header = raw2.slice(0, 12);
        var rawData = raw2.slice(12);
  ...
  var activityReader = class extends EventTarget {
    constructor(band) {
      super();
      this.Band = band;
      this.status = {
    ...
    async readSince(datetime, iterationCount) {
      ...
      await this.Band.Chars.FETCH.writeValueWithoutResponse(new Uint8Array([FETCH_COMMANDS.FROM_DATE, FETCH_DATA_TYPES.ACTIVITY_DATA, ...datetime, 0, 0]));
      await new Promise((resolve, reject) => {
        this.addEventListener("fetch_start", function(e) {
          resolve(e.detail);
        });
    ...
    async onFetchRead(e) {
      ...
      if (arraysEqual(raw2.slice(0, 3), new Uint8Array([16, 1, 1]))) {
        ...
        var year = convertToInt16Array([...raw2.slice(7, 7 + 2)].reverse())[0];
        var month = raw2[9] - 1;
        var day = raw2[10];
        var hour = raw2[11];
        if (raw2[14] > 0) {
          console.warn("Date Offset!");
        }
        var minute = raw2[12] - raw2[14] * 15;
        var second = raw2[13];
        this.rawStartDate = new Date(Date.UTC(year, month, day, hour, minute, second));
        this.dispatchEvent(new CustomEvent("fetch_start", {detail: true}));
        return true;
      } else if (arraysEqual(raw2.slice(0, 3), new Uint8Array([16, 2, 1]))) {
        await this.Band.Chars.FETCH.writeValueWithoutResponse(Uint8Array.from([3, 9]));
        ...
        if (nextTimestep < new Date()) {
          ...
          await this.readSince(nextTimestep, iterationCount + 1);
        } else {
  ...
  var Band7 = class {
    constructor(authKey) {
      ...
      this.Chars = {};
      this.handle = 0;
      this.Auth, this.sp02Reader, this.batteryReader = null;
    ...
    async onChunkedRead(e) {
      ...
      var raw2 = bufferToUint8Array(e.target.value);
      if (raw2.length >= 12) {
        var header = raw2.slice(0, 11 + 1);
        var endpoint = header.slice(9, 10 + 1).reverse();
        var command = header[11];
        if (convertToInt16Array(endpoint)[0], CHUNK_ENDPOINTS2.BATTERY) {
          if (command == CHUNK_RESPONSES2.RESULT_OK) {
            this.batteryReader.parseRaw(raw2);
          }
        }
      }
    }
    ...
    async writeChunkedValueFlags(char, type, handle, data, base_flags) {
      ...
      while (remaining > 0) {
        ...
        if (count == 0) {
          let i = 5;
          chunk[5] = data.length - flags & 255;
          chunk[6] = data.length - flags >> 8 & 255;
          chunk[7] = data.length - flags >> 16 & 255;
          chunk[8] = data.length - flags >> 24 & 255;
          chunk[9] = type & 255;
          chunk[10] = type >> 8 & 255;
          flags |= 1;
        }
    ...
    async writeChunkFlags(type, command, flags) {
      console.log(flags);
      await this.writeChunkedValueFlags(this.Chars.CHUNKED_WRITE, type, this.getNextHandle(), command, flags);
    }
    async writeChunk(type, command) {
      await this.writeChunkedValue(this.Chars.CHUNKED_WRITE, type, this.getNextHandle(), command);
    }
  ...
  var ActivityCategoryChart = class {
    constructor(selector) {
      this.category = [];
      this.steps = [];
      const options = {
